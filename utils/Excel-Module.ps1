function Export-Excel {
    ...
    process { if ($PSBoundParameters.ContainsKey("InputObject")) {
            ...
            foreach ($TargetData in $InputObject) {
                ...
                foreach ($Name in $script:Header) {
                    ...
                    try   {
                        ...
                        elseif ($v -isnot [String] -Or  [string]::IsNullOrEmpty($v)) { #Other objects or null.
                            if ($null -ne $v) { $ws.Cells[$row, $ColumnIndex].Value = $v.toString()}
                        }
                        elseif ($v[0] -eq '='){
                            if($NoFormulaConversion -ne '*' -and $NoFormulaConversion -notcontains $Name) {
                        ...
                        elseif ( [System.Uri]::IsWellFormedUriString($v , [System.UriKind]::Absolute) -and $NoHyperLinkConversion -ne '*' -and $NoNumberConversion -notcontains $Name) {
        ...
        if(-Not [string]::IsNullOrEmpty($ConditionalText) -Or -Not [string]::IsNullOrEmpty($ConditionalFormat)){
